...
export const AnswerItem = ({answerItem, deleteRequestAnswer}:{answerItem:AnswerType,deleteRequestAnswer:any}) =>{
    ...
    useEffect(()=>{
        setAnswerContent(answerItem.content)
    },[answerItem.content])
    return (
        <>
            <div className="p-4 mb-4 bg-white shadow-md rounded-md">
                <div className="text-gray-500 text-sm">ID: {answerItem.answerId}</div>
                <input className="mt-2 text-lg font-semibold" onChange={(e:any)=>setAnswerContent(e.target.value)} value={answerContent} disabled = {user?.nickname !== answerItem.nickname}/>
